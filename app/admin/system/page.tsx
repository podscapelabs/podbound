...
export default async function AdminSystemPage() {
  ...
  return <main id="main" className="admin-page shell">
    ...
    <section className={styles.statusGrid} aria-label="Current system status">
      <article className={`${styles.status} ${site?.maintenance_enabled ? styles.danger : styles.good}`}><span>Public website</span><strong>{site?.maintenance_enabled ? "Maintenance" : "Live"}</strong><small>{site?.maintenance_enabled ? "Visitor access paused" : "Available to visitors"}</small></article>
      <article className={`${styles.status} ${arena?.access_mode === "closed" ? styles.review : styles.good}`}><span>Arena access</span><strong>{fieldMode?.label || "Unknown"}</strong><small>Guest access disabled</small></article>
      <article className={`${styles.status} ${authConfigured ? styles.good : styles.danger}`}><span>Authentication</span><strong>{authConfigured ? "Configured" : "Attention needed"}</strong><small>Provider connection</small></article>
      <article className={`${styles.status} ${styles.review}`}><span>Email delivery</span><strong>{authEmailHourlyLimit}/hour</strong><small>Application sending budget</small></article>
    </section>

    <section className={styles.controlGrid} aria-label="Live access controls">
      <article className={`${styles.controlPanel} ${site?.maintenance_enabled ? styles.maintenanceActive : ""}`}><div className={styles.panelHeading}><div><p className="eyebrow">Emergency control</p><h2>Website availability</h2></div><span className={styles.recordMark} aria-hidden="true">01</span></div><p><strong>{site?.maintenance_enabled ? "Maintenance mode is active." : "The public website is live."}</strong></p><p className={styles.sectionIntro}>{site?.maintenance_enabled ? "Normal pages and APIs are unavailable. Administrator and sign-in routes remain accessible so you can restore service." : "Use this emergency switch to replace the public site with a maintenance notice."}</p><ConfirmForm action={setMaintenanceMode} message={site?.maintenance_enabled ? "Take the PodBound website live again?" : "Enable maintenance mode and make the PodBound website inaccessible to normal visitors?"}><input type="hidden" name="enabled" value={site?.maintenance_enabled ? "false" : "true"} /><button className={`button ${site?.maintenance_enabled ? "primary" : "secondary"}`}>{site?.maintenance_enabled ? "Take website live" : "Enable maintenance mode"}</button></ConfirmForm></article>

      <article className={`${styles.controlPanel} ${styles.accessPanel}`}><div className={styles.panelHeading}><div><p className="eyebrow">Simulator gate</p><h2>PodBound Arena access</h2></div><span className={styles.recordMark} aria-hidden="true">02</span></div><p className={styles.sectionIntro}>Choose who can enter the simulator. Guest entry remains disabled in every mode.</p><div className={styles.accessGrid}>{accessModes.map((mode) => <ConfirmForm key={mode.value} action={updateArenaMode} message={`Change Arena access to ${mode.label}?`}><input type="hidden" name="mode" value={mode.value} /><button className={`${styles.accessOption} ${arena?.access_mode === mode.value ? styles.active : ""}`}><strong>{mode.label}</strong><span>{mode.description}</span>{arena?.access_mode === mode.value && <small>Current setting</small>}</button></ConfirmForm>)}</div></article>
    </section>

    <section className={styles.readinessPanel}><div className={styles.panelHeading}><div><p className="eyebrow">Presence checks</p><h2>Configuration readiness</h2></div><span className={styles.recordMark} aria-hidden="true">03</span></div><p className={styles.sectionIntro}>These checks report presence only. Secret values are never rendered or sent to the browser.</p><div className={styles.configList}>{configChecks.map((check) => <article className={styles.configItem} key={check.label}><div><strong>{check.label}</strong><span>{check.detail}</span></div><span className={`${styles.configState} ${check.manual ? styles.review : check.ready ? styles.good : styles.danger}`}>{check.manual ? "Review required" : check.ready ? "Present" : "Missing"}</span></article>)}</div></section>

    <details className={styles.auditPanel}><summary><div><p className="eyebrow">Sensitive changes</p><h2>Administrator activity</h2><span>The latest {auditLogs.length} account, access, event, and maintenance records.</span></div><span className={styles.auditToggle}>Open log</span></summary><div className={styles.auditList}>{auditLogs.length ? auditLogs.map((log) => {
    ...
    }) : <p>No administrative changes have been recorded.</p>}</div></details>
  </main>;
}
